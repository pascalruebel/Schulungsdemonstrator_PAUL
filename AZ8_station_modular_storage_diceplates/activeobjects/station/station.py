...
class StorageStation(Actor):
    """ Storage Station class as an active object """
    def __init__(self, name, status_led, blinker, topics=None):
        super(StorageStation, self).__init__(name=name)
        self._name = name
        self._stationState = "notInitialized"
        self.__allowedStationStates = ('Starting', 'notInitialized', 'Standby', 'Ready', 'Running', 'Error')
        ...
        self.input_events = events.StorageStationInputEvent(events.StorageStationInputEvents.NoEvent, self._name )
        self.output_events = events.GenericServiceEvent(events.GenericServiceEvents.NoEvent, self._name)

        self._allowed_topics = ('StationState', 'Ack', 'StationErrorCode', 'StationErrorDescription', 'StationMessageCode', 'StationMessageDescription')
        if topics is None:
            self.topics = self._allowed_topics
        else:
            self.topics = topics
        ...
        self._stationStateMachine = StationStateMachine(self)  # state machine instance
    ...
    def stationState(self, new_stationState):
        if new_stationState not in self.__allowedStationStates:
            raise ValueError("Unknown staionState: {0}".format(new_stationState))
