...
from activeobjects.services.diceplate.diceplate_sm import ProvideDicePlateStateMachine
...
class ProvideDicePlateService(Actor):
    """ ProvideHalfDiceService class as an active object """
    def __init__(self, name, storageracklist, enable_timeout=False, timeout_interval=600, topics=None):
        super(ProvideDicePlateService, self).__init__(name=name)
        self._name = self.__class__.__name__
        self.enable_timeout = enable_timeout
        self.timeout_interval = timeout_interval
		
        self.allowed_topics = ('eventID', 'StationErrorCode', 'StationErrorDescription', 'StationMessageCode', 'StationMessageDescription', 'ProvideMaterialState')
        ...
        self.provideDicePlateStateMachine = ProvideDicePlateStateMachine(self, enable_timeout=self.enable_timeout, timeout_interval=self.timeout_interval, storageracklist=self._storageracklist)
    ...
    def handle_event(self, *args, **kwargs):
        self.provideDicePlateStateMachine.dispatch(*args, **kwargs)
