...
class Starting(StationState):
    ...
    def enter_action(self):
        ...
        # startup "progress bar" blinking
        self._station.blinkerLed1.handle_event(event=self._station_sm.blink_start_event)
        self._station.blinkerLed2.handle_event(event=self._station_sm.blink_start_event)
        self._station.blinkerLed3.handle_event(event=self._station_sm.blink_start_event)

        self._station.posled1.handle_event(event=self._station_sm.purple_event)
        self._station.posled2.handle_event(event=self._station_sm.purple_event)
        self._station.posled3.handle_event(event=self._station_sm.purple_event)
    ...
    def estop(self):
        ...
        self._station.blinkerLed1.handle_event(event=self._station_sm.blink_stop_event)
        self._station.blinkerLed2.handle_event(event=self._station_sm.blink_stop_event)
        self._station.blinkerLed3.handle_event(event=self._station_sm.blink_stop_event)
...
class NotInitialized(StationState):
    ...
    def enter_action(self):
        ...
        self._station.publisher.publish(topic='StationStateMaintenance', value="NotInitialized",
                                        sender=self._station.name)
        # check nfc-sensors usb-connection
        self._station.nfc1.handle_event(event=self._station_sm.sensor_checkconn_event)
        self._station.nfc2.handle_event(event=self._station_sm.sensor_checkconn_event)
        self._station.nfc3.handle_event(event=self._station_sm.sensor_checkconn_event)

        # LEDs off
        self._station.status_led.handle_event(event=self._station_sm.ledoff_event)
        self._station.posled1.handle_event(event=self._station_sm.ledoff_event)
        self._station.posled2.handle_event(event=self._station_sm.ledoff_event)
        self._station.posled3.handle_event(event=self._station_sm.ledoff_event)
...
class Estop(StationState):
    ...
    def enter_action(self):
        ...
        self._station.publisher.publish(topic='StationStateMaintenance', value="Error : RFID-NOK",
                                        sender=self._station.name)
...
class StationStateMachine(object):
    ...
    def __init__(self, station):
        ...
        self.sensor_checkconn_event = events.ComplexSensorEvent(eventID=events.ComplexSensorEvents.CheckConn, sender=self._station.name)
    ...
    def dispatch(self, *args, **kwargs):
        ...
        try:
            ...
            if sender == "RfidReader1" and topic == "State":
                if value == "StatusNOK":
                    self._station.posled1.handle_event(event=self.red_event)
                    self._current_state.estop()
                else:
                    self._station.posled1.handle_event(event=self.ledoff_event)
                    self._current_state.estop_ok()

            elif sender == "RfidReader2" and topic == "State":
                if value == "StatusNOK":
                    self._station.posled2.handle_event(event=self.red_event)
                    self._current_state.estop()
                else:
                    self._station.posled2.handle_event(event=self.ledoff_event)
                    self._current_state.estop_ok()

            elif sender == "RfidReader3" and topic == "State":
                if value == "StatusNOK":
                    self._station.posled3.handle_event(event=self.red_event)
                    self._current_state.estop()
                else:
                    self._station.posled3.handle_event(event=self.ledoff_event)
                    self._current_state.estop_ok()

        except KeyError:
            event = kwargs["event"]

            if event.eventID == events.StationInputEvents.Initialize:  # this a part of the service generic interface
                self._current_state.initialize()
            elif event.eventID == events.StationInputEvents.ToPosition1:
                self._current_state.service1()
            elif event.eventID == events.StationInputEvents.ToPosition2:
                self._current_state.service2()
            elif event.eventID == events.StationInputEvents.ToPosition3:
                self._current_state.service3()
            elif event.eventID == events.StationInputEvents.Ack:
                self._current_state.acknowledge()
                self.ack_error()
                self.ack_message()
            elif event.eventID == events.StationInputEvents.Done:
            ...
            elif event.eventID == events.StationInputEvents.Error:
                if event.init_required:
                    self.init_required = True
                self._current_state.error()

            elif event.eventID == events.StationInputEvents.NoConn:
                self._current_state.noconn()
            elif event.eventID == events.StationInputEvents.ConnOk:
                self._current_state.conn_ok()

            elif event.eventID == events.StationInputEvents.NoEvent:
