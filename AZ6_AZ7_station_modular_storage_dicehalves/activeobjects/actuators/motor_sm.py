...
class MotorStateMachine(object):
    ...
    def __init__(self, motor):
        ...
        self._current_state = self._notinit_state = NotInitialized(self, motor) # an instance for tracking current state

        self._standstill_state = Standstill(self, motor)
        self._rotating_cw_state = RotatingCW(self, motor)
        self._rotating_ccw_state = RotatingCCW(self, motor)
        self._estop_state = Estop(self, motor)
        self._error_state = Error(self, motor)  # motors's states instances
    ...
    def dispatch(self, *args, **kwargs):
        ...
        try:  # topics coming from the publishers
            ...
            if eventID == self._motor.input_events.eventIDs.Initialize:  # this a part of the service generic interface
                self._current_state.initialize()
            elif eventID == self._motor.input_events.eventIDs.Ack:
                self._current_state.acknowledge()
            elif eventID == self._motor.input_events.eventIDs.RotateCW:
                self._current_state.rotate_cw()
            elif eventID == self._motor.input_events.eventIDs.RotateCCW:
                self._current_state.rotate_ccw()
            elif eventID == self._motor.input_events.eventIDs.Stop:
                self._current_state.stop()
            elif eventID == self._motor.input_events.eventIDs.Error:
                self._current_state.error()
            elif eventID == self._motor.input_events.eventIDs.Timeout:
