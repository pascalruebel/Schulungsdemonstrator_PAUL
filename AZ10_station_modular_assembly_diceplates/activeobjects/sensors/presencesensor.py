...
class PresenceSensor(Actor):
    """ Presence sensor class as an active object """
    def __init__(self, name, id, topics, inputobj, auto_init=False, normally_open=True):
        super(PresenceSensor, self).__init__(name=name)
        self._name = name
        self._id = id

        # events:
        self.posedge_event = events.SimpleSensorInputEvent(eventID=events.SimpleSensorInputEvents.PosEdge, sender=self.name)
        self.negedge_event = events.SimpleSensorInputEvent(eventID=events.SimpleSensorInputEvents.NegEdge, sender=self.name)
