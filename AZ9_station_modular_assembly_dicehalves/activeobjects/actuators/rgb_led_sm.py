...
class RGB_LEDStateMachine(object):
    ...
    def __init__(self, led):
        ...
        self._notinit_state = NotInitialized(self, self._led) # an instance for tracking current state

        self._init_state = Initialized(self, self._led)
        self._redon_state = RedOn(self, self._led)
        self._greenon_state = GreenOn(self, self._led)
        self._yellowon_state = YellowOn(self, self._led)
        self._purpleon_state = PurpleOn(self, self._led)
        self._customon_state = CustomOn(self, self._led)
        self._ledoff_state = LedOff(self, self._led)
        self._error_state = Error(self, self._led)
    ...
    def publish_error(self, error_code):
        self._led.publisher.publish(topic="StationErrorCode",
                                              value=hex(error_code),
                                              sender=self._led.name)
        self._led.publisher.publish(topic="StationErrorDescription",
                                              value=error_codes.code_to_text[error_code],
                                              sender=self._led.name)

    def publish_message(self, message_code):
        self._led.publisher.publish(topic="StationMessageCode",
                                                value=hex(message_code),
                                                sender=self._led.name)
        self._led.publisher.publish(topic="StationMessageDescription",
