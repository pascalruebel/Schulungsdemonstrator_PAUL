...
class NotInitialized(SimpleSensorState):
    ...
    def initialize(self):
        ...
        self._sensor.init_check()

    def init_done(self):
        # self._sensor.logger.debug("%s event in %s state", self.init_done.__name__, self.name)
...
class SensorOn(SimpleSensorState):
    ...
    def enter_action(self):
        timeout_time = self._sensor.monitor_interval
        Timer(timeout_time, self._sensor_sm.current_state.update).start()
    ...
    def update(self):
        if self._sensor.is_greater_then_setpoint():
            timeout_time = self._sensor.monitor_interval
            Timer(timeout_time, self._sensor_sm.current_state.update).start()
        else:
            self._sensor.publisher.publish(topic="Value", value=False, sender=self._sensor.name)
...
class ForceSwitchStateMachine(object):
    ...
    def dispatch(self, *args, **kwargs):
        self._sensor.logger.debug("%s has received a message: %s in state %s" , self.name, kwargs, self._current_state.name)
        ...
        try:  # topics coming from the publishers
            ...
            if eventID == self._sensor.input_events.eventIDs.Initialize:  # this a part of the service generic interface
                ...
                self._current_state.acknowledge()
            elif eventID == self._sensor.input_events.eventIDs.InitCheckOk:
                self._current_state.init_done()
                ...
                self.set_state(self.update_state)
