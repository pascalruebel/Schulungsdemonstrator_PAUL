...
from activeobjects.services.pressing.press_service_sm import PressServiceStateMachine
from communication import events


import logging

class PressService(Actor):
    """ PressService class as an active object """
    def __init__(self, name, press, clamp, frontpos_sensor, presspos_sensor, blinker, enable_timeout=False, timeout_interval=600, topics=None):
        super(PressService, self).__init__(name=name)
        self._name = name
        ...
        self.pressServiceStateMachine = PressServiceStateMachine(self, self.enable_timeout, self.timeout_interval)
